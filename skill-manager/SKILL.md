...
以 `/Users/tangw/.agents/skills` 为唯一维护目录，以同目录的 `SKILLS.md` 为唯一人工清单。不得启动服务或创建定时任务。
...
8. 仅在验证成功且用户已授权本次安装时，在 `/Users/tangw/.agents/skills` 创建一个独立本地 Git 提交。
...
2. 将新版本下载到临时目录，完成与安装相同的安全检查并展示差异。
3. 等待用户明确确认具体版本。
4. 只替换该 skill，保留其他目录和软链接；验证失败则恢复旧内容。
5. 验证成功后更新 `SKILLS.md` 并创建单独的本地 Git 提交。
